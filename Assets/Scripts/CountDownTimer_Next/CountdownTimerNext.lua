...
function self:Update()
    ...
    for i = 1, #_eventStartEpochs do
        ...
        if now < s then
            -- future event countdown (unchanged)
            local rem = s - now
        ...
        elseif now < e then
            -- live now: either stay on this image or switch to next event
            if i + 1 > #_eventStartEpochs then
                -- no more events after this one
                print ("i = " .. i .. "num of event start epochs = " .. #_eventStartEpochs)
                countdownLabel:SetPrelocalizedText("LIVE NOW!")
                billboardDisplay.image = eventImages[i]
            else
                -- countdown until next event
                local remNext = _eventStartEpochs[i+1] - now
                local h = math.floor(remNext / 3600)
                local m = math.floor((remNext % 3600) / 60)
                local sec = remNext % 60
                local fmt = (h > 0) and "%02d:%02d:%02d" or "%02d:%02d"
                local text = (h > 0)
                    and string.format(fmt, h, m, sec)
                    or string.format(fmt, m, sec)
                countdownLabel:SetPrelocalizedText("Next Event: " .. text)
                billboardDisplay.image = eventImages[i+1]
            end
            return
        end
    end
