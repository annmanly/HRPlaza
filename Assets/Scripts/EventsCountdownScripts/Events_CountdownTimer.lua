...
function self:Update()
    ...
    for i = 1, maxCount do
        ...
        if now >= startEpoch and now < endEpoch then
            ...
            headerText:SetPrelocalizedText("LIVE NOW:")
            primaryText:SetPrelocalizedText(eventNames[i])
            countdownText:SetPrelocalizedText("EVENT ENDING IN: " .. formatted)
        ...
        elseif now < startEpoch then
            ...
            headerText:SetPrelocalizedText("COMING SOON:")
            primaryText:SetPrelocalizedText(eventNames[i])
            countdownText:SetPrelocalizedText("EVENT STARTING IN: " .. formatted)
            billboardDisplay.image = _eventTextures[i]
            return
        end
    end
